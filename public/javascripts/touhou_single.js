...
function load_songs() {
  if(pages == null) {
      $.getJSON('/songs/pages.json?search='+encodeURI(search_string), function(data) {
          songs_div.empty()
          pages = data
          if(pages == 0) {
            song_div.empty()
...
function load_page(page) {
  ...
  $.get("/songs?page=" + page + "&search="+encodeURI(search_string), null, null, "script")
}
...
function play_song(id) {
  ...
  $("#current_title").html(song.tracknum + ". " +song.title)
  $("#current_table").show()
...
function song_over() {
  ...
  if($("#queue_table tr").size() > 0) {
    ...
    if($("#after_stop").attr("checked")) {
      ...
      $.getJSON('/songs/'+(song.id)+'/next.json', function(data) {
          songs[data.id] = data
          play_song(data.id)
      })
    } else {
...
function resize_height() {
  ...
  $(".list_container").height($(window).height()-150)
}
...
$(function() {
  ...
  $("#search_form").submit(function() {
    ...
    search_string = jQuery.trim($("#search_field").val())
    current_page = 1
    pages = null
